...
export const ConversationList: React.FC<ConversationListProps> = ({
  ...
  const formatTimeFriendly = (date: string | Date) => {
    if (!date) return "";
    try {
      return formatDistanceToNow(new Date(date), { addSuffix: false, locale: vi });
    } catch {
      return "";
    }
  };
  ...
  return (
    ...
    >
      {/* Search Header */}
      <div className="p-4 space-y-3 bg-gradient-to-br from-orange-50/50 to-white border-b border-slate-100">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {_.isEmpty(conversations) ? (
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            ...
            <p className="text-sm font-medium text-slate-400">Không có cuộc trò chuyện nào</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-50">
            {_.map(conversations, (c) => {
              ...
              const lastTime = _.get(c, "lastModifiedDate") || _.get(c, "lastMessageAt");
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className={`text-sm truncate pr-2 ${hasUnread ? "font-bold text-slate-900" : "font-semibold text-slate-700"}`}>
                        {getShopName(c)}
                      </h4>
                    ...
                    <p className={`text-xs truncate ${hasUnread ? "text-slate-900 font-semibold" : "text-slate-500 font-normal"}`}>
