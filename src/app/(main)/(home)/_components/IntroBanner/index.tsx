...
const mapBannerToDisplay = (banner: BannerResponseDTO, index: number) => {
  ...
  return {
    ...
    imageUrlDesktop: banner.basePathDesktop
      ...
      : undefined,
    imageUrlMobile: banner.basePathMobile
      ? resolveBannerImageUrl(
...
export const IntroBanner = () => {
  ...
  useEffect(() => {
    ...
    const isExpired =
      !dismissedAt ||
      Date.now() - Number(dismissedAt) > INTRO_BANNER_EXPIRATION_MS;
    ...
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
      ...
      <div className="relative z-10000 w-full max-w-fit flex items-center justify-center animate-in zoom-in-95 duration-300">
        <button
          onClick={handleDismiss}
          className={cn(
            "absolute -top-3 -right-3 z-10001 bg-white text-gray-900",
            "rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center",
            "shadow-xl border border-gray-200 hover:scale-110 active:scale-95 transition-all"
          )}
        ...
        >
          {hasImage ? (
            <picture className="block">
              ...
              {bannerData.imageUrlDesktop && (
                <source
                  ...
                  srcSet={bannerData.imageUrlDesktop}
                />
              )}
              <img
                src={
            ...
            <div
              className={cn(
                "w-70 md:w-87.5 aspect-square rounded-3xl flex flex-col items-center justify-center text-white bg-linear-to-br shadow-2xl p-8 text-center",
                bannerData.gradient
              )}
            >
              ...
              <div className="mt-6 px-6 py-2 bg-white text-gray-900 rounded-full font-bold text-sm">
                XEM NGAY
              </div>
            </div>
          )}
