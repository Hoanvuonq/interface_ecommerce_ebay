    ...
    <SectionSreen id="calatha-mall" animation="slideUp">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/10 pb-6 mb-8">
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-yellow-500 text-[10px] font-black uppercase tracking-[0.3em]">Official Store</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-bold text-white tracking-tighter flex items-baseline gap-2">
            CALATHA <span className="italic font-light opacity-50">MALL</span>
          </h2>
        </div>
        ...
        <div className="flex items-center gap-8">
           <div className="hidden lg:flex items-center gap-6">
              {[
                { icon: <RotateCcw size={16} />, text: "7 Ngày Trả Hàng" },
                { icon: <ShieldCheck size={16} />, text: "100% Chính Hãng" },
                { icon: <Truck size={16} />, text: "Giao Hỏa Tốc" }
              ].map((item, idx) => (
                <div key={idx} className="flex items-center gap-2 text-[10px] font-bold text-white/70 uppercase tracking-tighter italic">
                  <span className="text-yellow-500">{item.icon}</span>
                  {item.text}
                </div>
              ))}
           </div>
           <Link href="/products" className="group flex items-center gap-2 px-5 py-2.5 bg-white/5 border border-white/10 rounded-full text-white text-[11px] font-bold uppercase hover:bg-white hover:text-black transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="lg:col-span-4 relative group min-h-[400px] rounded-[2.5rem] overflow-hidden shadow-2xl"
        >
          <Image
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-10 flex flex-col justify-end">
            <h3 className="text-white text-5xl font-black leading-none mb-4 tracking-tighter italic">
              PREMIUM<br/>
              <span className="text-yellow-500 underline decoration-4 underline-offset-8">EXPERIENCE.</span>
            </h3>
            <p className="text-white/60 text-xs font-medium mb-6 max-w-[200px] leading-relaxed">
              Trải nghiệm mua sắm đẳng cấp với các thương hiệu quốc tế ưu đãi độc quyền.
            </p>
            <Link href="/products" className="w-fit bg-yellow-600 text-white px-8 py-3.5 rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-yellow-500 transition-all active:scale-95 shadow-2xl shadow-yellow-600/20">
              MUA NGAY
            </Link>
          </div>
        </motion.div>
        ...
        <div className="lg:col-span-8 overflow-x-auto scrollbar-none snap-x snap-mandatory flex lg:grid lg:grid-cols-4 gap-4 pb-4">
          {displayProducts.map((product: any, idx: number) => (
            <div key={product.id || idx} className="min-w-[180px] sm:min-w-[220px] lg:min-w-0 snap-start shrink-0">
              <BrandCard
                product={product}
          ...
          <Link
            href="/products"
            className="min-w-[180px] lg:min-w-0 snap-start shrink-0 flex flex-col items-center justify-center bg-white/5 border border-dashed border-white/20 rounded-[2rem] hover:bg-white/10 transition-all group p-6"
          >
            ...
            <span className="text-[11px] font-black text-white/80 uppercase tracking-[0.2em] text-center italic">
              Khám phá<br/>thế giới mall
            </span>
          </Link>
        </div>
      </div>
