        ...
        <div className="lg:w-[32%] flex flex-col gap-3">
          {[sideBanner1, sideBanner2].map((banner, idx) => (
            <Link
              key={idx}
              href={banner.href}
              className="relative flex-1 group rounded-3xl overflow-hidden border border-slate-100 shadow-sm transition-all duration-500 hover:shadow-blue-200/50"
            >
              <img
              ...
              <div className="absolute top-3 right-3 px-3 py-1 bg-white/20 backdrop-blur-md rounded-full border border-white/30 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
                 <span className="text-[10px] font-bold text-white uppercase tracking-widest">Discover</span>
              </div>
            </Link>
          ))}
        </div>
