          ...
          <div className={cn("relative", sidebar ? "grid grid-cols-1 lg:grid-cols-4 gap-10" : "")}>
            {sidebar && (
              <div className="lg:col-span-1 hidden lg:block sticky top-24 h-fit group">
                <div className="bg-white border border-slate-100 rounded-[2.5rem] p-4 shadow-sm group-hover:shadow-xl transition-all duration-500">
                  {sidebar}
                </div>
              </div>
            )}
                ...
                <AnimatePresence>
                  {currentPage > 0 && (
                    <motion.button
                      ...
                      className="absolute -left-6 top-1/2 -translate-y-1/2 z-40 w-14 h-14 bg-white rounded-full shadow-xl border border-slate-50 flex items-center justify-center text-slate-900 hover:text-orange-500 transition-all active:scale-90"
                    >
                  ...
                  {(currentPage + 1) * pageSize < rawProducts.length && (
                    <motion.button
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      onClick={() => setCurrentPage((p) => p + 1)}
                      className="absolute -right-6 top-1/2 -translate-y-1/2 z-40 w-14 h-14 bg-white rounded-full shadow-xl border border-slate-50 flex items-center justify-center text-slate-900 hover:text-orange-500 transition-all active:scale-90"
                    >
