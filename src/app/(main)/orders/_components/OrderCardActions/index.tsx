      ...
      {isPendingPayment && paymentUrl ? (
        <>
          <Link
            href={paymentUrl}
            target="_blank"
            className="w-full py-3 bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold uppercase tracking-widest rounded-xl text-center shadow-lg shadow-orange-200 transition-all active:scale-95"
          >
            Thanh toán ngay
          </Link>
          {canCancel && (
        ...
        <div className="flex gap-2 w-full">
          {firstProductId && (
            <Link href={`/products/${firstProductId}`} className="flex-1">
              <button className="w-30 py-3 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold uppercase tracking-widest rounded-xl transition-all shadow-md">
                Mua lại
              </button>
            </Link>
          )}
