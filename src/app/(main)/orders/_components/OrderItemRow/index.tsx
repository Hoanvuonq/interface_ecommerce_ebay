...
export const OrderItemRow = ({
  ...
  return (
    <div className="p-4 sm:p-5 flex gap-4 sm:gap-6 hover:bg-gray-50/50 transition-all duration-300 group border-b border-gray-50 last:border-0">
      
      <div className="relative w-20 h-20 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-gray-100 shrink-0 bg-white shadow-sm flex items-center justify-center transition-transform group-hover:scale-[1.02]">
        {imageUrl ? (
          <Image 
            src={imageUrl} 
      ...
      <div className="flex-1 min-w-0 flex items-center justify-between py-1">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[9px] font-bold text-gray-600 uppercase tracking-widest bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
              SKU: {item.sku || "N/A"}
            </span>
            <span className="text-gray-600 text-[9px] font-medium uppercase">
              Số lượng: <b className="text-gray-700">{item.quantity}</b>
            </span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-end mt-2">
          ...
          {canReview && (
            ...
            >
              <Star
                size={12}
                className={cn(isReviewed ? "fill-gray-200" : "fill-current")}
              />
