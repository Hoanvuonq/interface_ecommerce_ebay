...
const normalizeWishlistName = (name: string): string => {
    if (name?.toLowerCase() === 'deauft' || name?.toLowerCase() === 'default') return 'Danh sách mặc định';
    return name;
};
...
export interface WishlistListItemProps {
    ...
    onTogglePrivacy: (wishlist: WishlistSummaryResponse, e?: React.MouseEvent) => void;
    onSetAsDefault: (wishlist: WishlistSummaryResponse, e?: React.MouseEvent) => void;
...
export const WishlistListItem = memo<WishlistListItemProps>(({
    ...
    const coverImageUrl = useMemo(() => {
        return resolveCoverImageUrl(wishlist.imageBasePath, wishlist.imageExtension, '_thumb');
        ...
        <motion.div
            ...
            className={cn(
                ...
                isSelected 
                    ...
                    : "border-slate-100 bg-white hover:border-orange-200 hover:shadow-lg hover:shadow-slate-200/50"
            )}
        >
            <div className="flex items-center gap-4">
                ...
                <div className="relative shrink-0">
                    {coverImageUrl ? (
                        <div className="h-14 w-14 overflow-hidden rounded-xl border border-slate-100 shadow-sm">
                            <img
                                src={coverImageUrl}
                                alt={wishlist.name}
                        ...
                        <div className={cn(
                            ...
                            isSelected ? "bg-orange-500 text-white" : "bg-slate-50 text-slate-400 group-hover:bg-orange-50 group-hover:text-orange-500"
                        )}>
                            <Heart size={24} fill={isSelected ? "currentColor" : "none"} strokeWidth={isSelected ? 1.5 : 2} />
                        </div>
                    )}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                        ...
                        <ChevronRight 
                            ...
                            className={cn(
                                "transition-all",
                                isSelected ? "translate-x-0 text-orange-500 opacity-100" : "-translate-x-2 text-slate-300 opacity-0 group-hover:translate-x-0 group-hover:opacity-100"
                            )} 
                        />
                    </div>
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                        ...
                        >
                            {wishlist.isPublic ? <Unlock size={12} strokeWidth={3} /> : <Lock size={12} strokeWidth={3} />}
