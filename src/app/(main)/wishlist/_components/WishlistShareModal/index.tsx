...
export default function WishlistShareModal({
  ...
  const handleShare = (platform: "fb" | "tw" | "wa") => {
    ...
    const encodedText = encodeURIComponent(
      `Xem danh sách yêu thích của mình: ${wishlistName}`,
    );
        ...
        {!shareToken ? (
          <div className="py-10 text-center space-y-4 bg-gray-50 rounded-4xl border border-dashed border-gray-200">
            ...
            <p className="text-gray-500 font-medium leading-relaxed px-6">
              ...
              <span className="text-orange-600 font-bold">Công khai</span> để
              chia sẻ.
            </p>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center pt-2">
              ...
              <div className="space-y-4 text-center md:text-left">
                <label className="text-[10px] font-semibold uppercase  text-gray-600 ml-1 block">
                  Mạng xã hội
                </label>
            ...
            <div className="pt-6 border-t border-gray-100">
              <button
                ...
                onClick={async () => {
                  if (
                    confirm(
                      "Link cũ sẽ không còn hoạt động. Bạn có chắc chắn muốn tạo link mới?",
                    )
                  ) {
                    try {
                      setIsRegenerating(true);
                      await onRegenerateToken();
                      toastSuccess("Đã làm mới liên kết!");
                    } finally {
