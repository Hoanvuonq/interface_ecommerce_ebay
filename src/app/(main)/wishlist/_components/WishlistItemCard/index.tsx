...
export const WishlistItemCard = memo<WishlistItemCardProps>(({
    ...
    const imageUrl = useMemo(() => {
        const resolvedUrl = item.imageBasePath && item.imageExtension 
            ? resolveVariantImageUrl({ imageBasePath: item.imageBasePath, imageExtension: item.imageExtension }, '_medium')
            : '';
    ...
    const priorityStyles = useMemo(() => {
        switch (item.priority) {
            case 1: return "bg-orange-500 text-white border-orange-500 shadow-orange-100";
            case 2: return "bg-red-600 text-white border-red-600 shadow-red-100";
        ...
        >
            ...
            <div className="relative aspect-square overflow-hidden bg-[#fafafa]">
                ...
                {!readOnly && (
                    <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-10">
                        <button 
                            onClick={() => onEdit?.(item)}
                            className="p-3 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl text-slate-700 hover:text-orange-500 hover:scale-110 transition-all"
                        >
                            <Edit3 size={18} />
                        </button>
                        <button 
            ...
            <div className="flex-1 p-6 flex flex-col">
                <Link href={`/products/${item.productId}`} className="block mb-3">
                    <h3 className="text-sm font-bold text-slate-800 line-clamp-2 hover:text-orange-500 transition-colors leading-snug h-10">
                        {item.productName}
                    </h3>
                </Link>
                ...
                <div className="mt-auto pt-4 border-t border-slate-50 flex items-center justify-between">
                    ...
                    <button
                        ...
                        onClick={() => onAddToCart(item.variantId, item.productName, item.id)}
                        className="w-12 h-12 bg-orange-500 hover:bg-slate-900 disabled:bg-slate-100 text-white rounded-2xl flex items-center justify-center transition-all duration-300 active:scale-90 shadow-lg shadow-orange-200 group/btn"
                    >
                        {isAddingToCart ? (
            ...
            <AnimatePresence>
                {showDeleteConfirm && (
                    <motion.div 
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 z-50 bg-slate-900/40 backdrop-blur-[4px] p-6 flex items-center justify-center"
                    >
                        <motion.div 
                            initial={{ scale: 0.9, y: 10 }}
                            animate={{ scale: 1, y: 0 }}
                            className="bg-white rounded-[2rem] p-6 shadow-2xl w-full text-center border border-slate-100"
                        >
