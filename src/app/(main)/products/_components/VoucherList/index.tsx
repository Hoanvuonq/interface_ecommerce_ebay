...
const CustomSpinner: React.FC<any> = ({ size = "small" }) => {
  ...
  return (
    <div className="flex items-center justify-center">
      <Loader2
        className={`w-${currentSize} h-${currentSize} animate-spin text-blue-500`}
      />
    </div>
  );
};
...
export function VoucherList({ product }: VoucherListProps) {
  ...
  const loadVouchers = useCallback(async () => {
    ...
    try {
      ...
      const shopVouchersList: VoucherDisplay[] = shopVouchersData.map(
        (v: any) => ({
          id: v.id,
          code: v.code,
          name: v.description,
          discountType:
            v.discountType === "PERCENTAGE" ? "PERCENTAGE" : "FIXED_AMOUNT",
          discountValue:
            v.discountType === "PERCENTAGE" ? v.discountAmount : undefined,
          discountAmount:
            v.discountType === "FIXED" ? v.discountAmount : undefined,
  ...
  const renderVoucherItem = (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p
          className={cn(
            "font-medium text-gray-800 line-clamp-1",
      ...
      <CustomButton
        ...
        onClick={() => alert(`Đã lưu voucher ${voucher.code}`)}
      >
        Lưu
      </CustomButton>
    </div>
