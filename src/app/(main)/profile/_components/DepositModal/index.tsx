...
export const DepositModal: React.FC<DepositModalProps> = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 sm:p-6 animate-fade-in font-sans">
      ...
      <div className={`relative w-full bg-white rounded-2xl shadow-2xl transform transition-all flex flex-col overflow-hidden border border-gray-100 z-10 ${
          ...
          {!depositResponse ? (
            <div className="space-y-6">
              <div className="bg-orange-50 border border-orange-100 rounded-xl p-4 flex gap-3 shadow-sm">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                  <p className="font-bold text-orange-800 mb-1">Lưu ý khi nạp tiền</p>
                  <ul className="list-disc list-inside space-y-1 ml-1 opacity-90 text-xs sm:text-sm">
                    <li>Số tiền nạp tối thiểu: <strong>1,000 VND</strong>.</li>
                    <li>Nội dung chuyển khoản phải <strong>chính xác</strong>.</li>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    type="button"
                    onClick={handleClose}
                    className="flex-1 py-3 text-sm font-bold text-gray-600 bg-gray-50 hover:bg-gray-100 hover:text-gray-800 rounded-xl transition-colors"
                  >
                    Hủy bỏ
                  </button>
                  <button
                    type="submit"
                    disabled={loading}
                    className="flex-2 py-3 text-sm font-bold text-white bg-linear-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 rounded-xl shadow-lg shadow-orange-200 transition-all transform active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {loading ? <FiLoader className="animate-spin" /> : "Tạo mã QR"}
                  </button>
                </div>
              </form>
            </div>
            ...
            <div className="space-y-6 animate-fade-in-right">
              <div className="text-center space-y-1">
                ...
                <h4 className="text-lg font-bold text-gray-900">Yêu cầu đã được tạo!</h4>
                <p className="text-sm text-gray-500">Quét mã bên dưới để thanh toán ngay</p>
              </div>
                    ...
                    {depositResponse.qrCode ? (
                        <>
                            ...
                            <p className="mt-3 text-xs font-medium text-gray-400 text-center">
                                Hết hạn lúc: {new Date(depositResponse.expiresAt).toLocaleTimeString('vi-VN', {hour: '2-digit', minute:'2-digit'})}
                            </p>
                        </>
                    ) : (
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
                        ...
                        <div className="flex justify-between items-center text-sm">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="font-mono font-bold text-gray-900 tracking-wide">{depositResponse.accountNumber}</span>
                                <button onClick={() => handleCopy(depositResponse.accountNumber || "", "Số tài khoản")} className="text-orange-500 hover:bg-orange-50 p-1 rounded transition-colors"><FiCopy size={14}/></button>
                            </div>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500">Chủ tài khoản</span>
                            <span className="font-bold text-gray-900 uppercase">{depositResponse.accountName}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-start text-sm">
                            ...
                            <div className="flex items-center gap-2 max-w-35 justify-end">
                                ...
                                <button onClick={() => handleCopy((depositResponse as any).description || description, "Nội dung chuyển khoản")} className="text-orange-500 hover:bg-orange-50 p-1 rounded transition-colors shrink-0"><FiCopy size={14}/></button>
                            </div>
                        </div>
                    </div>
