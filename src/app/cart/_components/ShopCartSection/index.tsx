    ...
    return (
        <section className="mb-6 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden transition-all hover:shadow-md">
                ...
                <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="relative flex items-center justify-center">
                        <input 
                            type="checkbox"
                            id={`shop-${shop.shopId}`}
                            className="peer appearance-none w-5 h-5 border-2 border-gray-200 rounded-md checked:bg-orange-500 checked:border-orange-500 transition-all cursor-pointer"
                    ...
                    <div className="flex items-center gap-3 min-w-0 group cursor-pointer">
                        <div className="relative shrink-0">
                            <img 
                                ...
                                onError={(e) => {
                                    (e.target as HTMLImageElement).src = `https://ui-avatars.com/api/?name=${encodeURIComponent(shop.shopName)}&background=f3f4f6&color=6b7280`;
                                }}
                            />
                        ...
                        <div className="flex flex-col min-w-0">
                            <div className="flex items-center gap-2">
                                ...
                                <ChevronRight size={14} className="text-gray-300 group-hover:text-orange-400 transition-transform group-hover:translate-x-0.5" />
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                                {shop.rating && shop.rating > 0 && (
                                    <div className="flex items-center gap-0.5 px-1.5 py-0.5 bg-amber-50 rounded-md">
                                        ...
                                        <span className="text-[10px] font-bold text-amber-700">{shop.rating.toFixed(1)}</span>
                                    </div>
                                )}
                                {shop.isVerified && (
            ...
            <div className="hidden lg:block">
                <div className="grid grid-cols-12 px-6 py-3 bg-gray-50/50 text-[10px] font-black uppercase tracking-[0.1em] text-gray-400 border-b border-gray-50">
            ...
            <div className="lg:hidden divide-y divide-gray-50 bg-white">
                {shop.items.map((item) => (
                    <div key={item.id} className="p-2 transition-colors active:bg-gray-50">
                        <CartItem
                            item={item}
                            etag={etag}
                            selected={item.selectedForCheckout}
                ...
                {shop.discount > 0 && (
                    <div className="px-5 py-2.5 flex justify-between items-center bg-orange-50/40 border-b border-orange-100/50">
                        <div className="flex items-center gap-2 text-orange-600">
                            ...
                            <span className="text-[11px] font-bold uppercase tracking-tight">Cửa hàng giảm giá:</span>
                        </div>
