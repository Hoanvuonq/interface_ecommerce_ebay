...
import {
  ...
  useGetCategoryById,
  useUpdateCategory,
} from "@/app/manager/_hooks/useCategoryController";
import { SectionHeader, FormInput, CustomButtonActions } from "@/components";
...
export const UpdateForm: React.FC<UpdateFormProps> = ({
  ...
  const handleImageFileChange = async (
    ...
    if (!file.type.startsWith("image/"))
      return showToast("Chỉ chấp nhận file hình ảnh", "error");
    if (file.size / 1024 / 1024 > 5)
      return showToast("Ảnh tối đa 5MB", "error");
    ...
    <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden max-w-5xl mx-auto animate-in zoom-in-95 duration-500">
      ...
      <div className="bg-linear-to-br from-indigo-600 to-purple-700 px-10 py-10 text-white relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-5">
          ...
          <div>
            <h2 className="text-3xl font-semibold uppercase tracking-tighter italic leading-none">
              Update{" "}
              <span className="text-indigo-100 underline decoration-4 underline-offset-8">
                Entity
              </span>
            </h2>
            <p className="text-indigo-100 text-xs font-bold uppercase tracking-[0.3em] mt-3 opacity-80 italic">
              Revision Hash: {category.id.slice(0, 12).toUpperCase()}
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-10 bg-[#fffcf9]">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-4 space-y-4">
            <SectionHeader icon={ImageIcon} title="Visual Asset" />
            <div
            ...
            >
              {previewImage || currentImageUrl ? (
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <Loader2
                      className={cn(
                        ...
                        uploadingImage ? "animate-spin" : "hidden",
                      )}
                    />
                ...
                <Plus
                  size={48}
                  className="text-gray-200 group-hover:text-indigo-400 transition-colors"
                />
              )}
              <input
                type="file"
          ...
          <div className="lg:col-span-8 space-y-8">
            <SectionHeader icon={Layout} title="Metadata Configuration" />
        ...
        <div className="space-y-6 pt-6 border-t border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-semibold text-gray-800 uppercase tracking-tighter italic text-[11px] font-semibold text-gray-600 uppercase tracking-[0.2em]">
              Shipping <span className="text-blue-600">Protocol</span>
            </h3>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-8 rounded-[40px] border-2 border-gray-100 shadow-sm">
            <div className="space-y-2">
              <label className="text-[9px] font-semibold text-gray-600 uppercase  ml-2 leading-none mb-1.5">
                Restriction Type
              </label>
              <select
                ...
                onChange={(e) =>
                  setFormData((p) => ({
                    ...
                    defaultShippingRestrictions: {
                      ...p.defaultShippingRestrictions!,
                      restrictionType: e.target.value as any,
                    },
                  }))
                }
            ...
            {formData.defaultShippingRestrictions?.restrictionType ===
              "LOCAL_RADIUS" && (
              <div className="space-y-2 animate-in slide-in-from-right">
                ...
                <input
                  ...
                  value={
                    formData.defaultShippingRestrictions?.maxShippingRadiusKm ||
                    ""
                  }
                  ...
                  onChange={(e) =>
                    setFormData((p) => ({
                      ...
                      defaultShippingRestrictions: {
                        ...p.defaultShippingRestrictions!,
                        maxShippingRadiusKm: Number(e.target.value),
                      },
                    }))
                  }
                />
        ...
        <CustomButtonActions
          ...
          submitText="Push Revisions"
          cancelText="Discard"
          hasChanges={true}
          containerClassName="rounded-b-[40px]"
        />
      </form>
    </div>
  );
