...
class AdminCampaignService {
     ...
     */
    async getAllCampaigns(page = 0, size = 10): Promise<PagedResponse<CampaignResponse>> {
     ...
     */
    async getCampaignsByStatus(status: CampaignStatus, page = 0, size = 10): Promise<PagedResponse<CampaignResponse>> {
        const response: ApiResponse<PagedResponse<CampaignResponse>> = await request({
     ...
     * Update campaign
     */
    async updateCampaign(campaignId: string, req: UpdateCampaignRequest): Promise<CampaignResponse> {
        const response: ApiResponse<CampaignResponse> = await request({
     ...
     * Cancel campaign
     */
    async cancelCampaign(campaignId: string, reason: string): Promise<CampaignResponse> {
        const response: ApiResponse<CampaignResponse> = await request({
     ...
     */
    async createSlots(campaignId: string, req: CreateSlotsRequest[]): Promise<CampaignSlotResponse[]> {
     ...
     */
    async getPendingRegistrations(page = 0, size = 10): Promise<PagedResponse<CampaignSlotProductResponse>> {
    ...
    /**
     * Get pending registrations for a campaign
     */
    async getPendingRegistrationsForCampaign(campaignId: string, page = 0, size = 10): Promise<PagedResponse<CampaignSlotProductResponse>> {
        const response: ApiResponse<PagedResponse<CampaignSlotProductResponse>> = await request({
     ...
     */
    async approveRegistration(registrationId: string): Promise<CampaignSlotProductResponse> {
     ...
     */
    async rejectRegistration(registrationId: string, reason: string): Promise<CampaignSlotProductResponse> {
        const response: ApiResponse<CampaignSlotProductResponse> = await request({
     ...
     */
    async batchReject(registrationIds: string[], reason: string): Promise<{ rejected: number }> {
