...
import {
    Shield, Plus, Calendar, Tag, Clock, Zap,
    ChevronRight, RefreshCw, CheckCircle, XCircle,
    Trash2, Eye, BarChart3, Users, Package,
    Play, Ban, Edit
} from 'lucide-react';
...
export default function AdminCampaignDemoPage() {
    ...
    const [pendingRegistrations, setPendingRegistrations] = useState<CampaignSlotProductResponse[]>([]);
    const [selectedCampaign, setSelectedCampaign] = useState<CampaignResponse | null>(null);
    const [campaignStats, setCampaignStats] = useState<CampaignStatisticsResponse | null>(null);
    const [selectedRegistrations, setSelectedRegistrations] = useState<string[]>([]);

    // UI state
    const [loading, setLoading] = useState(false);
    const [activeTab, setActiveTab] = useState<'campaigns' | 'registrations' | 'statistics'>('campaigns');
    ...
    const fetchData = async () => {
        ...
        try {
            const [campaignsRes, registrationsRes] = await Promise.all([
                adminCampaignService.getAllCampaigns(0, 20).catch(() => ({ content: [] })),
                adminCampaignService.getPendingRegistrations(0, 20).catch(() => ({ content: [] })),
            ]);
    ...
    const handleQuickCreateCampaign = async () => {
        ...
        // if (!confirm('Tạo nhanh một campaign bắt đầu NGAY BÂY GIỜ để test?')) return;
        ...
        try {
            ...
            // Format dates as YYYY-MM-DD for backend if needed, or simple ISO string depending on DTO
            // Assuming backend accepts ISO string or LocalDate (YYYY-MM-DD)
            // Using ISO String for safety with most modern backends, or local YYYY-MM-DD
            const formatDatePayload = (d: Date) => d.toISOString().split('T')[0];
            ...
            await adminCampaignService.createCampaign({
                ...
                startDate: formatDatePayload(now),
                endDate: formatDatePayload(threeDaysLater),
                isRecurring: false,
                slotDurationMinutes: 520, // To cover generous time
                slotTimes: ['09:00', '12:00', '15:00', '18:00', '21:00'], // Typical slots
                displayPriority: 1
            });
        ...
        return (
            <div className="min-h-screen bg-gray-50 p-8">
                <div className="max-w-4xl mx-auto text-center">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 mb-2">Admin Campaign Demo</h1>
                    <p className="text-gray-500 mb-6">Quản lý campaigns và duyệt đăng ký</p>

                    <div className="bg-white rounded-xl border border-gray-200 p-8 shadow-sm max-w-md mx-auto">
                        <Shield className="w-12 h-12 text-amber-500 mx-auto mb-4" />
                        <h3 className="font-semibold text-gray-900 mb-2">Yêu cầu quyền Admin</h3>
                        <p className="text-sm text-gray-500 mb-6">
                            Vui lòng đăng nhập với tài khoản <strong>Admin</strong> để truy cập trang quản trị này.
                        </p>
                        <a
                            href="/login"
        ...
        <div className="min-h-screen bg-gray-50">
                ...
                <div className="max-w-7xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="font-bold text-xl text-gray-900">Admin Campaign</h1>
                                <p className="text-sm text-gray-500">Quản lý Platform Campaigns</p>
                            </div>
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-4">
                <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit">
                    {[
                        { id: 'campaigns', label: 'Campaigns', icon: Calendar, badge: campaigns.length },
                        { id: 'registrations', label: 'Duyệt đăng ký', icon: Package, badge: pendingRegistrations.length },
                        ...
                        >
                            ...
                            {badge !== undefined && badge > 0 && (
                                <span className={`px-2 py-0.5 rounded-full text-xs ${activeTab === id
                                ...
                                <table className="w-full">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-left text-sm font-medium text-gray-600">Stats</th>
                                ...
                                {campaignStats ? (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                                            ...
                                            <p className="text-2xl font-bold text-gray-900">{campaignStats.totalSlots}</p>
                                            <p className="text-sm text-gray-500">{campaignStats.activeSlots} đang active</p>
                                        </div>
                                        ...
                                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                                            ...
                                            <p className="text-2xl font-bold text-gray-900">{campaignStats.totalRegistrations}</p>
                                            <p className="text-sm text-gray-500">{campaignStats.approvedRegistrations} đã duyệt</p>
                                        </div>
                                        ...
                                        <div className="bg-white rounded-xl p-4 border border-gray-200">
                                            ...
                                            <p className="text-2xl font-bold text-gray-900">{campaignStats.totalSold}</p>
