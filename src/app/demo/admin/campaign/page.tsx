...
export default function AdminCampaignDemoPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toastSuccess("Đã tải dữ liệu từ API");
    } catch (err: any) {
  ...
  const handleApproveRegistration = async (regId: string) => {
    try {
      ...
      toastSuccess("Đã duyệt đăng ký");
      fetchData();
    } catch (err: any) {
      toastError(err.message || "Không thể duyệt");
    }
  };
  ...
  const handleBatchApprove = async () => {
    if (selectedRegistrations.length === 0) {
      toastError("Chưa chọn đăng ký nào");
      return;
    }

    try {
      const result = await adminCampaignService.batchApprove(
        selectedRegistrations,
      );
      toastSuccess(`Đã duyệt ${result.approved} đăng ký`);
  ...
  const handleCancelCampaign = async (campaignId: string) => {
    ...
    try {
      ...
      toastSuccess("Đã hủy campaign");
      fetchData();
    } catch (err: any) {
      toastError(err.message || "Không thể hủy");
    }
  };
  ...
  const handleQuickCreateCampaign = async () => {
    ...
    try {
      ...
      toastSuccess("Đã tạo campaign thành công (Start: NOW)");
      fetchData();
    } catch (err: any) {
      toastError(err.message || "Không thể tạo campaign");
    }
  };
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              {campaign.status === "DRAFT" && (
                                <button
                                  onClick={() =>
                                    handleScheduleCampaign(campaign.id)
                                  }
                              ...
                              {["DRAFT", "SCHEDULED", "ACTIVE"].includes(
                                campaign.status,
                              ) && (
                                <button
                                  onClick={() =>
                ...
                {pendingRegistrations.map((reg) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <img
                        src={
                          reg.productThumbnail || "https://picsum.photos/80/80"
                        }
                        alt={reg.productName}
