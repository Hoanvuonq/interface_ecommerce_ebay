...
import type { PublicProductDetailDTO, PublicProductListItemDTO, PublicProductVariantDTO } from "@/types/product/public-product.dto";
import type { ReviewResponse, ReviewStatisticsResponse } from "@/types/reviews/review.types";
...
export const ProductDetailProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  // 1. Logic detect Slug/ID (Fix: Phải dùng regex để nhận diện chính xác như code cũ)
  ...
  // 2. Main Data Fetching (Gộp logic fetch product, featured và reviews)
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      // Gọi đúng API dựa trên Slug hoặc ID
      const res = isSlug 
      ...
      // Set default variant (Ưu tiên variant có ảnh như code cũ)
      const defaultV = data?.variants?.find((v: any) => v.imageUrl || (v.imageBasePath && v.imageExtension)) ?? data?.variants?.[0] ?? null;
      setSelectedVariant(defaultV);

      if (data?.id) {
        // Fetch Review đầu tiên (size 3)
        const rev = await getProductReviewComments(data.id, { page: 0, size: 3 });
        setProductReviews(rev.content || []);
        setReviewHasMore(!rev.last);

        // Fetch Featured Products (loại trừ sp hiện tại)
  ...
  const handleOpenShopChat = async (): Promise<void> => {
    ...
    try {
      ...
      if (!response?.success) {
        toast.error(response?.message || "Lỗi khởi tạo chat");
      }
    } catch (error) {
      toast.error("Không thể kết nối server chat");
    } finally {
  ...
  const loadMoreReviews = async (): Promise<void> => {
    ...
    try {
      const res = await getProductReviewComments(product.id, { page: nextPage, size: 3 });
  ...
  // 5. Logic Gallery & Pricing (Giữ nguyên useMemo bạn đã viết)
  const priceInfo = useMemo(() => {
    ...
    const calculateVoucherPrice = () => {
      ...
      if (bestVoucher.discountType === "PERCENTAGE")
        return Math.max(0, vPrice - (vPrice * bestVoucher.discountValue!) / 100);
    ...
    return {
      ...
      primaryPrice: pPrice,
      comparePrice: cPrice,
      discountPercentage: cPrice && cPrice > pPrice ? Math.round(((cPrice - pPrice) / cPrice) * 100) : null,
      priceRangeLabel: product?.priceMin !== product?.priceMax ? `₫${product?.priceMin} - ₫${product?.priceMax}` : null,
    };
  ...
  const galleryImages = useMemo(() => {
    if (!product) return [];
    const items: any[] = [];
    product.media?.filter((m) => m.type === "IMAGE").forEach((m) => {
      const url = resolveMediaUrlHelper(m as any, "_large");
      items.push({ key: `media-${m.id}`, thumb: resolveMediaUrlHelper(m as any, "_thumb") || url, preview: url });
    });
    product.variants?.forEach((v) => {
      const url = resolveVariantImageUrlHelper(v as any, "_large");
      if (url) items.push({ key: `variant-${v.id}`, thumb: resolveVariantImageUrlHelper(v as any, "_thumb") || url, preview: url });
    });
    return items.filter((v, i, a) => a.findIndex((t) => t.preview === v.preview) === i);
  }, [product]);

  const primaryImage = useMemo(() => {
    if (activeThumbnail) return activeThumbnail;
    const vImg = resolveVariantImageUrlHelper(selectedVariant as any, "_large");
    return vImg && vImg !== "/placeholder-product.png" ? vImg : galleryImages[0]?.preview || PLACEHOLDER_IMAGE;
  }, [activeThumbnail, selectedVariant, galleryImages]);

  useEffect(() => { setActiveThumbnail(null); }, [selectedVariant]);

  const value = {
    product, loading, featured, selectedVariant, setSelectedVariant,
    productReviews, reviewSummary: product?.reviewStatistics || null,
    reviewsLoading, reviewHasMore, loadMoreReviews,
    creatingShopChat, handleOpenShopChat,
    activeThumbnail, setActiveThumbnail, galleryImages, primaryImage,
    priceInfo,
...
export const useProductDetail = () => {
  ...
  if (!context) throw new Error("useProductDetail must be used within Provider");
  return context;
};