...
export const ProductCard = ({
    ...
    const handleAddToCart = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            const variant = resp.data.variants?.find((v: any) => v.stock > 0) || resp.data.variants?.[0];
        ...
        <>
            ...
            >
                ...
                <div className={cn(
                    ...
                    isList ? "w-32 md:w-44 h-full border-r border-gray-50" : "aspect-square w-full"
                )}>
                    {imageUrl && !imgError ? (
                        <img
                            alt={product.name}
                            src={imageUrl}
                            // Thêm các thuộc tính optimize hiển thị ảnh sắc nét
                            className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 antialiased"
                            style={{ imageRendering: 'auto' }}
                            onError={() => setImgError(true)}
                        />
                    ) : (
                        <div className={cn("w-full h-full flex flex-col items-center justify-center gap-2", categoryUI.colors.bg)}>
                            <span className={isList ? "text-2xl" : "text-5xl"}>{categoryUI.emoji}</span>
                            <span className={cn("text-[8px] font-black uppercase tracking-widest opacity-40", categoryUI.colors.text)}>
                                No Image
                            </span>
                        </div>
                    )}
                    ...
                    {/* Nút thêm nhanh chỉ hiện ở Grid mode */}
                    {!isList && (
                        <div className="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-black/20 to-transparent hidden sm:block">
                            <button
                                onClick={handleAddToCart}
                                disabled={addingToCart || product.active === false}
                                className="w-full bg-white/90 backdrop-blur-md cursor-pointer hover:bg-orange-500 hover:text-white text-gray-900 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all flex items-center justify-center gap-2"
                            >
                                {addingToCart ? <Loader2 size={12} className="animate-spin" /> : <ShoppingBag size={14} />}
                                Thêm vào giỏ
                            </button>
                        </div>
                    )}
                ...
                <div className={cn(
                    ...
                    <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-1.5 overflow-hidden">
                            <span className="bg-orange-50 text-orange-600 text-[8px] font-black px-1.5 py-0.5 rounded border border-orange-100 uppercase shrink-0 tracking-tighter">Mall</span>
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-tight truncate">CaLaTha Store</span>
                        </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isList ? "text-gray-300 hover:text-red-500" : "absolute top-2 right-2 bg-white/80 backdrop-blur-md shadow-sm hover:bg-white hover:text-red-500 z-20"
                            )}
                        >
                            {loadingProductDetail ? (
                                ...
                                <Heart size={isList ? 16 : 14} className="text-gray-400 group-hover:text-red-500" />
                            )}
                        </button>
                    </div>
                    ...
                    <h3 className={cn(
                        ...
                        isList ? "text-sm md:text-base line-clamp-1" : "text-sm line-clamp-2 h-10"
                    )}>
                        {product.name}
                    </h3>

                    <div className={cn("mt-auto", isList && "flex items-end justify-between gap-3")}>
                        <div className="flex-1">
                            <div className="flex items-baseline gap-2 mb-0.5">
                                <span className={cn("font-black text-orange-600 tracking-tighter leading-none", isList ? "text-lg md:text-xl" : "text-lg")}>
                            ...
                            <div className="flex items-center gap-2">
                                <div className="flex items-center gap-0.5">
                                    ...
                                    <span className="text-[10px] font-bold text-gray-600">4.5</span>
                                </div>
                        ...
                        {isList && (
                            <div className="hidden sm:block">
                                <CustomButton 
                                    variant="dark" 
                                    className="!h-9 !px-4 !text-[10px] uppercase tracking-wider shadow-md"
                                    onClick={handleAddToCart}
