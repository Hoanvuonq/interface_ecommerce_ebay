...
// ❌ Loại bỏ Antd Imports
import { toast } from 'sonner'; // ✅ Thay thế Antd message
import {
    ShoppingCart, Flame, Star, Loader2, // ✅ Lucide Icons
} from 'lucide-react'; 
...
interface ProductShowcaseProps {
    title?: string;
    subtitle?: string;
    rows?: number; // Number of rows to display (default: 4)
}
...
export const ProductShowcase: React.FC<ProductShowcaseProps> = ({
    ...
    const [currentPage, setCurrentPage] = useState<{ sale: number, new: number }>({ sale: 0, new: 0 }); 
    const [hasMore, setHasMore] = useState<{ sale: boolean, new: boolean }>({ sale: true, new: true }); 
    ...
    // Fetch products
    const fetchProducts = useCallback(async (type: 'sale' | 'new', page: number) => {
        try {
            ...
            const productsList = (response.data?.content || []) as PublicProductListItemDTO[];

            if (productsList.length > 0) {
                if (type === 'sale') {
                    // 🎯 Sửa lỗi Implicit any: Khai báo kiểu cho prev: PublicProductListItemDTO[]
                    setSaleProducts((prev: PublicProductListItemDTO[]) => page === 0 ? productsList : [...prev, ...productsList]);
                } else {
                    // 🎯 Sửa lỗi Implicit any: Khai báo kiểu cho prev: PublicProductListItemDTO[]
                    setNewProducts((prev: PublicProductListItemDTO[]) => page === 0 ? productsList : [...prev, ...productsList]);
                }
                ...
                const currentTotal = type === 'sale'
                    ? (page === 0 ? productsList.length : saleProducts.length + productsList.length)
                    : (page === 0 ? productsList.length : newProducts.length + productsList.length);
                ...
                // 🎯 Sửa lỗi Implicit any: Khai báo kiểu cho prev: { sale: boolean, new: boolean }
                setHasMore((prev: { sale: boolean, new: boolean }) => ({ ...prev, [type]: false }));
            }
        } catch (error) {
            console.error(`Error fetching ${type} products:`, error);
            toast.error(`Không thể tải sản phẩm ${type === 'sale' ? 'giảm giá' : 'mới'}`); // ✅ Thay thế message.error
        }
    }, [productsPerPage, checkVariantsInWishlist, saleProducts.length, newProducts.length]);
    ...
    const handleLoadMore = async () => {
        ...
        setCurrentPage((prev: { sale: number, new: number }) => ({ ...prev, [type]: nextPage }));
    ...
    const canLoadMore = activeTab !== 'all' && hasMore[activeTab as 'sale' | 'new'];
        ...
        },
        {
            ...
            label: (
                <span className="flex items-center gap-2 px-2 text-red-500">
                    <Flame className="w-4 h-4 fill-red-500" /> {/* ✅ Thay thế FireOutlined */}
                    Giảm giá
                </span>
            ),
        },
