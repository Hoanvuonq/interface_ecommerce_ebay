...
import { MODE_CONFIG, UniversalLoginFormProps } from "../../_constants/formLogin";
...
export function UniversalLoginForm({ mode }: UniversalLoginFormProps) {
  ...
  useEffect(() => {
    ...
    const keys = [config.storageKeyUser, config.storageKeyEmail, config.storageKeyPass];
  ...
  const handleSocialLogin = async (loginType: "GOOGLE" | "FACEBOOK") => {
    ...
    try {
      const res = await handleLoginSocial({ loginType, role: config.role as any });
      if (res?.success && res?.data) window.location.href = res.data;
      else toastError("Lỗi khởi tạo", { description: res?.message || "Không thể kết nối MXH." });
    } catch (err) {
      toastError("Lỗi kết nối", { description: "Kiểm tra lại đường truyền mạng." });
    } finally {
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8">
          <div className="w-full max-w-md relative">
            ...
            <div className="w-full shadow-2xl bg-white/90 dark:bg-gray-800/90 border-2 border-pink-500/30 p-8 sm:p-10 rounded-3xl backdrop-blur-md">
              ...
              <form onSubmit={handleSubmit(onFinish)} className="space-y-4 mb-6">
                <InputField 
                    label="Tên đăng nhập" 
                    name="username" 
                ...
                <div className="text-right">
                  <Link className="text-orange-600 hover:text-orange-700 text-sm font-medium" href={config.forgotPassLink}>Quên mật khẩu?</Link>
                </div>
              ...
              <div className="text-center space-y-3 pt-4 border-t border-gray-100">
                <p className="text-gray-600 text-sm">
                  ...
                  <Link className="text-orange-600 font-semibold" href={config.registerLink}>Đăng ký ngay</Link>
                </p>
                <Link href={config.homeLink} className="inline-flex items-center gap-2 text-gray-500 hover:text-gray-700 text-sm transition-colors">
            ...
            {AUTH_PANEL_DATA[config.panelType] && (
              <MobileFeatureList features={AUTH_PANEL_DATA[config.panelType].features} />
            )}
          </div>
        </div>
      </div>
