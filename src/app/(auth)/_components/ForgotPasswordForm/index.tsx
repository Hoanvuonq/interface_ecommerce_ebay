...
export default function ForgotPasswordForm() {
  ...
  const handleFinish = async (e: React.FormEvent) => {
    ...
    if (step === 1) {
      ...
      try {
        const res = await handleVerifyPassword({
          email: formData.email,
          otpCode,
        });
        if (res) {
          ...
          toastError("Xác thực thất bại", {
            description:
              verifyError || "Mã OTP không chính xác hoặc đã hết hạn.",
          });
          resetOtp();
        }
      } catch (err: any) {
    ...
    switch (step) {
      case 1:
        return {
          ...
          title: "Quên mật khẩu?",
          desc: "Nhập email đã đăng ký để nhận mã xác thực",
        };
      case 2:
        return {
          ...
          title: "Xác minh OTP",
          desc: "Nhập mã OTP vừa được gửi đến email của bạn",
        };
      case 3:
        return {
          icon: FaLock,
          ...
          desc: "Tạo mật khẩu mới an toàn cho tài khoản",
        };
      default:
        return {
          icon: FaKey,
          ...
          <div className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-md rounded-4xl shadow-2xl border border-white/20 p-8 sm:p-10 transition-all">
            <div className="text-center mb-8">
              <div className="flex justify-center mb-6">
                <div
                  className={cn(
                    "w-20 h-20 bg-linear-to-br rounded-3xl rotate-12 flex items-center justify-center shadow-xl shadow-orange-500/20",
                    config.gradient
                  )}
                >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        copyToClipboard(formData.password).then(() =>
                          success("Đã sao chép!")
                        )
                      }
                      className="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 rounded-xl text-[10px] font-bold text-slate-600 dark:text-slate-200 transition-all uppercase"
            ...
            <div className="mt-10 pt-8 border-t border-gray-100 dark:border-slate-700">
              <div className="flex items-center gap-4 p-4 bg-amber-50/50 dark:bg-amber-900/10 rounded-2xl">
                ...
                <div className="text-[11px] text-amber-800 dark:text-amber-300 font-bold leading-tight">
                  Tuyệt đối không chia sẻ mã OTP hoặc mật khẩu với bất kỳ ai, kể
                  cả nhân viên hệ thống CaLaTha.
                </div>
              </div>
            </div>
          </div>
