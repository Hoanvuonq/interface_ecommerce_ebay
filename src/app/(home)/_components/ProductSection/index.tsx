...
const ProductSection: React.FC<ProductSectionProps> = ({
  ...
  const fetchProducts = useCallback(async (page: number, size: number = 24, isInitial: boolean = false) => {
    try {
      isInitial ? setLoading(true) : setLoadingMore(true);
      let response;
      if (type === "featured") response = await publicProductService.getFeatured(page, size);
      else if (type === "sale") response = await publicProductService.getSale(page, size);
      ...
      if (list.length > 0) {
        ...
        setHasMore(products.length + list.length < (response.data?.totalElements || 0));

        if (isAuthenticated()) {
          const vIds = list.map((p) => ("variants" in p && p.variants?.[0]?.id ? p.variants[0].id : "")).filter((id) => id);
          if (vIds.length > 0) {
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-6 border-b border-slate-100 pb-8 px-2">
          ...
          {showViewAll && (
            <Link
              href={viewAllHref}
              className="group inline-flex items-center gap-3 px-6 py-3 bg-slate-50 hover:bg-orange-500 rounded-2xl text-[11px] font-black uppercase tracking-[0.1em] text-slate-600 hover:text-white transition-all duration-300 shadow-sm shadow-slate-200/50"
            >
              XEM TẤT CẢ
          ...
          <div className={cn("relative", sidebar ? "lg:col-span-3" : "w-full")}>
            {loading ? (
              ...
              <div className="relative group/grid">
                {/* Navigation Buttons - Styled like the screenshot */}
                <AnimatePresence>
                  {currentPage > 0 && (
                    <motion.button
                      key="prev-btn"
                  ...
                  {(startIndex + pageSize < products.length || hasMore) && (
                    <motion.button
                      ...
                      exit={{ opacity: 0, x: 20 }}
                      onClick={() => {
                        if (startIndex + pageSize >= products.length) fetchProducts(currentPage + 1, pageSize, false);
                    ...
                    >
                      {loadingMore ? <Loader2 className="w-6 h-6 animate-spin" /> : <ChevronRight size={28} strokeWidth={2.5} />}
                    </motion.button>
                  )}
                </AnimatePresence>
                ...
                >
                  {paginatedProducts.map((product) => {
                    const vId = ("variants" in product && product.variants?.[0]?.id) || "";
                    return (
                      <motion.div
                        key={product.id}
