...
const PurchasedVoucherList: React.FC<PurchasedVoucherListProps> = ({
    onCountUpdate,
}) => {
    ...
    const getStatusInfo = (instance: VoucherInstance) => {
        ...
        if (instance.remainingQuantity === 0) {
            return { color: "bg-gray-100 text-gray-500", label: "Đã dùng hết", icon: <Inbox size={12} /> };
        }
        if (expiry.isBefore(now)) {
            return { color: "bg-red-100 text-red-600", label: "Đã hết hạn", icon: <Clock size={12} /> };
        }
        if (daysLeft <= 3) {
            return { color: "bg-amber-100 text-amber-700", label: `Còn ${daysLeft} ngày`, icon: <AlertCircle size={12} /> };
        }
        return { color: "bg-emerald-100 text-emerald-700", label: "Đang hoạt động", icon: <CheckCircle2 size={12} /> };
    };

    const getScopeIcon = (scope: string) => {
        switch (scope) {
            case "SHOP_ORDER": return { icon: <ShoppingBag size={14} />, label: "Đơn hàng", color: "text-blue-600 bg-blue-50" };
            case "SHIPPING": return { icon: <Truck size={14} />, label: "Miễn ship", color: "text-emerald-600 bg-emerald-50" };
            case "PRODUCT": return { icon: <Package size={14} />, label: "Sản phẩm", color: "text-purple-600 bg-purple-50" };
            default: return { icon: <Zap size={14} />, label: scope, color: "text-gray-600 bg-gray-50" };
        }
    };
        ...
        <div className="space-y-6 animate-in fade-in duration-700">
            ...
            <div className="bg-amber-50/50 border border-amber-100 rounded-3xl p-5 flex gap-4 items-start shadow-sm">
                ...
                <div className="space-y-1">
                    <h4 className="text-sm font-bold text-amber-900 uppercase tracking-widest">Lưu ý vận hành</h4>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                <AnimatePresence mode="popLayout">
                                    {instances.map((item, index) => {
                                        ...
                                        const usagePercent = Math.round(((item.totalQuantity - item.remainingQuantity) / item.totalQuantity) * 100);
                                            ...
                                            >
                                                <td className="px-6 py-5">
                                                    <div className="flex flex-col gap-1.5">
                                                        ...
                                                        <div className="text-sm font-bold text-gray-800 line-clamp-1">{item.voucherName}</div>
                                                        <div className={cn("flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-tighter w-fit", scope.color)}>
                                                            {scope.icon}
                                                            {scope.label}
                                                        </div>
                                                    </div>
                                                ...
                                                <td className="px-6 py-5">
                                                    <div className="w-32 space-y-2">
                                                        ...
                                                        <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                            <div 
                                                                className={cn("h-full transition-all duration-1000", usagePercent === 100 ? "bg-red-400" : "bg-orange-500")}
                                                ...
                                                <td className="px-6 py-5">
                                                    <span className={cn("inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-[10px] font-bold uppercase tracking-tight", status.color)}>
                                                        {status.icon}
                                                        {status.label}
                                                    </span>
                                                </td>
...
export default PurchasedVoucherList;