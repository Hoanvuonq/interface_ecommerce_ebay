  ...
  const footerContent = (
    <div className="flex items-center justify-end gap-3 w-full">
      <Button
        variant="edit"
        onClick={onClose}
        className="h-11! px-10 rounded-2xl font-bold uppercase text-[10px] tracking-[0.2em]"
      >
        Hủy
      </Button>
      <ButtonField
        ...
        <form onSubmit={onFormSubmit} className="p-6 space-y-4">
          <div className="bg-blue-50/50 border border-blue-100 p-3 rounded-3xl flex items-start gap-4">
            ...
            <p className="text-[10px] font-bold text-blue-800 leading-relaxed uppercase tracking-tight">
              Lưu ý: Voucher sau khi phát hành sẽ được công khai. Hãy kiểm tra
              kỹ các thông số giảm giá và thời gian áp dụng.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
                ...
                <div className="flex p-1 bg-gray-50 rounded-2xl border border-gray-100 w-fit">
                  {["manual", "auto"].map((mode) => (
                    <button
                      key={mode}
                      type="button"
                      onClick={() => setCodeGenerateMode(mode as any)}
                      className={cn(
                        "px-8 py-2.5 text-[10px] font-bold uppercase rounded-xl transition-all",
                        codeGenerateMode === mode
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="relative group">
                  <FormInput
                    ...
                    onChange={(e) =>
                      setFormData((p: any) => ({ ...p, code: e.target.value }))
                    }
                    className="font-mono uppercase h-12! rounded-2xl!"
                  ...
                  {codeGenerateMode === "auto" && (
                    <button
                      ...
                      className="absolute right-3 top-9 p-2 text-orange-500 hover:bg-orange-50 rounded-xl transition-all z-10"
                    >
                      <RefreshCw size={18} />
                    </button>
                  )}
              ...
              <FormInput
                ...
                placeholder="Ghi chú về voucher này..."
              />
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <div className="flex p-1 bg-gray-50 rounded-2xl border border-gray-100">
                    {[
                      { id: DiscountType.FIXED_AMOUNT, label: "Số tiền" },
                      { id: DiscountType.PERCENTAGE, label: "Phần trăm" },
                    ].map((t) => (
                      <button
                        key={t.id}
                        type="button"
            ...
            <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex p-1 bg-gray-50 rounded-2xl border border-gray-100">
                  {[
                    ...
                    { id: false, label: "Sản phẩm chọn lọc" },
                  ].map((opt) => (
                    <button
                      key={String(opt.id)}
                      type="button"
              ...
              {!formData.applyToAllProducts && (
                <div className="animate-in slide-in-from-top-2 duration-300">
                  <select
                    multiple
                    className="w-full px-5 py-4 rounded-3xl border border-gray-200 bg-gray-50/50 text-[10px] font-bold uppercase outline-none h-40 custom-scrollbar appearance-none focus:bg-white transition-all"
                    onChange={(e) =>
                      setFormData((p: any) => ({
                        ...p,
                    ...
                    {products
                      ...
                      .map((p) => (
                        <option
                          key={p.id}
                          value={p.id}
                          className="py-3 px-5 rounded-xl mb-1 checked:bg-orange-500 checked:text-white"
                        >
                          {p.name}
                        </option>
                      ))}
