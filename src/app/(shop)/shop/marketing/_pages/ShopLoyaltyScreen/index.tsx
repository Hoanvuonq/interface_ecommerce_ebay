...
export const ShopLoyaltyScreen: React.FC = () => {
  ...
  const handlePolicyChange = useCallback((policy: LoyaltyPolicyResponse | null) => {
    setHasPolicy(!!policy);
  }, []);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 font-medium italic mt-1">
                Gia tăng lòng trung thành và thúc đẩy doanh số bằng tích lũy điểm
              </p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[600px]">
        ...
        <div className="flex border-b border-slate-50 px-8 pt-6">
          <div className="flex gap-8">
            <button
              ...
              className={cn(
                "relative pb-4 flex items-center gap-2 text-sm font-black uppercase tracking-widest transition-all",
                activeTab === "policy" ? "text-orange-600" : "text-slate-400 hover:text-slate-600"
              )}
            >
              <Settings size={16} />
              Chính sách Shop
            ...
            <button
              ...
              className={cn(
                "relative pb-4 flex items-center gap-2 text-sm font-black uppercase tracking-widest transition-all",
                activeTab === "products" ? "text-orange-600" : "text-slate-400 hover:text-slate-600"
              )}
            >
              <Gift size={16} />
              Khuyến mãi Sản phẩm
        ...
        <div className="p-8">
          ...
          <div className={cn(
            ...
            <div className={cn(
              "mt-1 p-2 rounded-xl",
              activeTab === "policy" ? "bg-orange-100 text-orange-600" : "bg-blue-100 text-blue-600"
            )}>
              <Info size={18} />
            </div>
            <div>
              <h4 className={cn(
                ...
                {activeTab === "policy" ? "Quy tắc tích điểm mặc định" : "Cấu hình riêng theo sản phẩm"}
              </h4>
              <p className={cn(
                "text-xs mt-1 leading-relaxed font-medium",
                activeTab === "policy" ? "text-orange-800/70" : "text-blue-800/70"
              )}>
                {activeTab === "policy" 
          ...
          <div className="animate-in fade-in zoom-in-95 duration-500">
            {activeTab === "policy" ? (
              ...
              <ProductPromotionList onCountUpdate={handlePromotionCountUpdate} />
            )}
          </div>
        </div>
      </div>
