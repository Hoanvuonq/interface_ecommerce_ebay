...
import { DataTable, SearchComponent } from "@/components";
...
export const ProductPromotionList: React.FC<ProductPromotionListProps> = ({
  onCountUpdate,
}) => {
  ...
  const [editingPromotion, setEditingPromotion] =
    useState<ProductLoyaltyPromotionResponse | null>(null);
  ...
  const handleToggle = async (promotionId: string) => {
    try {
      ...
      setPromotions((prev) =>
        prev.map((p) => (p.id === promotionId ? result : p)),
      );
          ...
          <div className="relative w-12 h-12 rounded-xl overflow-hidden bg-slate-100 border border-slate-200 shrink-0">
            {record.productThumbnail ? (
              ...
              <div className="w-full h-full flex items-center justify-center">
                <Gift className=" text-gray-300" size={20} />
              </div>
            )}
          </div>
      ...
      render: (record) => (
        <div className="flex flex-col">
          <div className="flex items-center gap-1.5  text-gray-700 font-bold">
            ...
            <span>
              {record.ruleType === "FIXED"
                ? `+${record.ruleValue} điểm`
                : `+${record.ruleValue}% giá`}
            </span>
          </div>
        ...
        >
          <span
            className={cn(
              ...
              record.enabled ? "translate-x-5" : "translate-x-1",
            )}
          />
        </button>
      ),
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-4xl border border-slate-100 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs  text-gray-400 font-medium italic mt-1">
              Tích điểm thưởng khi mua các sản phẩm cụ thể
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="p-2.5  text-gray-500 hover:bg-slate-50 rounded-xl border border-slate-200 transition-all active:scale-95"
          >
            <RefreshCw size={18} className={cn(loading && "animate-spin")} />
          </button>
          <button
            onClick={() => setIsFormOpen(true)}
            className="flex items-center gap-2 px-6 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold uppercase tracking-widest rounded-xl shadow-lg shadow-orange-200 transition-all active:scale-95"
          >
        ...
        headerContent={
          <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
            <SearchComponent
              ...
              size="md"
              className="relative group min-w-70"
            />
            <select
