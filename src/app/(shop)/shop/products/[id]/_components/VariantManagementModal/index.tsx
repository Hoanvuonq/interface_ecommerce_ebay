  ...
  const footer = (
    <div className="flex items-center justify-between w-full">
      ...
      <div className="flex items-center gap-3">
        <button onClick={onClose} className="px-6 py-2.5 text-sm font-bold text-gray-400 hover:text-gray-600">
          Đóng
        </button>
        <button
          onClick={onSave}
      ...
      title={
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest leading-none mt-1">{productName}</p>
          </div>
        </div>
      }
      footer={footer}
        ...
        <table className="w-full text-sm border-collapse bg-white">
          <thead>
            <tr className="bg-gray-50/50 border-b border-gray-100">
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest w-12">#</th>
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest w-24">Media</th>
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest min-w-40">Thông tin SKU</th>
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest w-40">Giá bán (₫)</th>
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest w-28">Kho</th>
              <th className="px-4 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest min-w-50">Phân loại</th>
              <th className="px-4 py-4 text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest w-20">Xóa</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {manageRows.map((row, idx) => (
              <tr key={row.rowKey} className={cn("hover:bg-orange-50/30 transition-colors", row.isNew && "bg-orange-50/10")}>
                <td className="px-4 py-4 text-gray-400 font-bold text-xs">{idx + 1}</td>
                <td className="px-4 py-4 text-center">
                   <label className="relative block w-14 h-14 rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50 overflow-hidden cursor-pointer group/img">
                      ...
                      {row.imageUrl ? (
                        ...
                        <ImageIcon size={20} className="text-gray-300 absolute inset-0 m-auto" />
                      )}
                   ...
                   <div className="flex flex-wrap gap-2">
                      {optionNames.map(name => (
                        <div key={name} className="flex flex-col gap-1">
                          <span className="text-[9px] font-bold text-gray-400 uppercase ml-1">{name}</span>
                          <input 
