...
export const ShopAddressCard = ({ item, index, onEdit, onDelete }: Props) => {
  ...
  return (
    <div className="group relative bg-white border border-slate-200 rounded-4xl p-6 hover:border-orange-400 hover:shadow-[0_20px_40px_rgba(255,115,36,0.1)] transition-all duration-500 overflow-hidden">
      ...
      <span className="absolute -bottom-4 -right-2 text-[80px] font-black text-slate-50 opacity-[0.05] select-none group-hover:text-orange-500 group-hover:opacity-[0.08] transition-all duration-700 italic leading-none">
        {index + 1}
      </span>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5 relative z-10">
        {item.default && (
          <span className="flex items-center gap-1 px-3 py-1 bg-emerald-50 text-emerald-600 text-[9px] font-black uppercase rounded-full border border-emerald-100 shadow-sm">
            <CheckCircle2 size={10} strokeWidth={3} /> Mặc định
          </span>
        )}
        {item.defaultPickup && (
          <span className="flex items-center gap-1 px-3 py-1 bg-orange-50 text-orange-600 text-[9px] font-black uppercase rounded-full border border-orange-100 shadow-sm">
            <Truck size={10} strokeWidth={3} /> Lấy hàng
          </span>
        )}
        {item.defaultReturn && (
          <span className="flex items-center gap-1 px-3 py-1 bg-amber-50 text-amber-600 text-[9px] font-black uppercase rounded-full border border-amber-100 shadow-sm">
      ...
      <div className="space-y-4 relative z-10">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <span className="font-black text-slate-800 text-sm uppercase tracking-tight italic leading-none">
              {item.fullName}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <span className="text-sm font-black text-slate-700 tracking-tighter leading-none">
              {item.phone}
            </span>
          </div>
        </div>
      ...
      <div className="mt-6 pt-5 border-t border-slate-100 flex justify-end gap-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500 ease-out">
        <button
          onClick={() => onEdit(item)}
          className="flex items-center gap-1.5 px-4 py-2 bg-slate-900 text-white rounded-xl hover:bg-orange-500 transition-all font-black text-[10px] uppercase tracking-wider shadow-md active:scale-95"
        >
        ...
        {!isProtected && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-all font-black text-[10px] uppercase tracking-wider active:scale-95"
          >
