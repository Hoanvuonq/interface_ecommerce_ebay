...
import { ACTION_OPTIONS, ACTIONS_BY_TAB } from "../../_constants/option";
    ...
    <div className="space-y-6 bg-white p-7 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/40 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="flex items-center gap-8 border-b border-gray-100 overflow-x-auto no-scrollbar">
        {subTabs.map((tab) => (
          <button
            ...
            className={cn(
              "pb-4 text-[13px] font-bold transition-all relative whitespace-nowrap tracking-tight",
      ...
      <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-y-4 gap-x-10">
          <div className="flex items-center gap-4">
            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest min-w-17.5">
              Ưu tiên
            </span>
            <div className="flex gap-2">
              {["Tất cả", "Hết hạn sau 1 ngày", "Hết hạn sau 2 ngày"].map(
                (p) => (
                  <button
                    key={p}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {currentActions.length > 3 && (
                <button
                  ...
                  className="text-orange-500 text-[12px] font-bold ml-2 hover:underline"
                >
                  {isExpanded
                    ? "Thu gọn"
                    : `+${currentActions.length - 3} khác`}
                </button>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 pt-6 border-t border-gray-50">
        <div className="lg:col-span-5 relative group">
          ...
          />
          <Search
            className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-orange-500 transition-colors"
            size={20}
          />
        </div>
        ...
        <div className="lg:col-span-3 flex gap-3">
          <button className="flex-2 bg-slate-900 hover:bg-black text-white font-bold rounded-2xl transition-all shadow-xl shadow-gray-200 active:scale-95 text-[11px] uppercase tracking-widest flex items-center justify-center gap-2">
          ...
          <button
            className={cn(
              ...
              "bg-white hover:bg-gray-50 text-gray-400 rounded-2xl transition-all active:scale-95 border-2 border-gray-100 group",
            )}
          >
            <RotateCcw
              size={20}
