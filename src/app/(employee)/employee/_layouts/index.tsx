...
export const BaseEmployeeLayout = ({
  ...
  const { collapsed, setCollapsed, isMobile, isMounted } =
    useResponsiveLayout();
  ...
  if (!isMounted) return <div className="h-screen w-full bg-[#f5f7ff]" />;

  return (
    <div className="flex h-screen bg-[#f5f7ff]  text-gray-800 antialiased">
      {!isMobile && (
        <aside
          style={{ width: `${sidebarWidth}px` }}
          className="h-full transition-[width] duration-300 ease-in-out shrink-0 border-r border-gray-200 bg-white z-40 relative"
        >
