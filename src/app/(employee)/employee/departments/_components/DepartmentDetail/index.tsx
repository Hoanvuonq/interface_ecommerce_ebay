...
export default function DepartmentDetail({
  ...
  const [department, setDepartment] = useState<DepartmentDetailType | null>(null);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 bg-slate-50/50 p-6 rounded-4xl border border-slate-100 flex flex-col justify-between">
              ...
              <div className="flex gap-4 mt-6">
                <BadgeInfo icon={<Users size={14}/>} label="Nhân sự" value={department.totalEmployees || 0} color="text-emerald-600" bgColor="bg-emerald-50" />
                <BadgeInfo icon={<Briefcase size={14}/>} label="Chức vụ" value={department.totalPositions || 0} color="text-purple-600" bgColor="bg-purple-50" />
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-4xl border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="space-y-3">
                ...
                <MetaItem icon={<FaUserEdit size={12}/>} label="Người quản lý" value={department.createdBy || "System"} />
                <MetaItem icon={<Calendar size={12}/>} label="Cập nhật cuối" value={dayjs(department.lastModifiedDate).format("DD/MM/YYYY")} />
              </div>
            </div>
          </div>
