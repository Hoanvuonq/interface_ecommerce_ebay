...
export default function DepartmentStatistics() {
  const { handleGetDepartmentStatistics, loading } =
    useGetDepartmentStatistics();
  ...
  const fetchStatistics = async () => {
    try {
      ...
      if (res?.data) {
        ...
        const employeesArray = Object.entries(data.employees || {})
          .map(([name, count]) => ({
            name,
            value: count as number,
          }))
  ...
  if (loading) return <SectionLoading message="Đang phân tích dữ liệu..." />;
    ...
    <div className="min-h-screen space-y-4 animate-in fade-in duration-700">
      <div className="space-y-2">
        ...
        <p className="text-gray-600 text-xs font-bold uppercase tracking-widest">
          Báo cáo cấu trúc nhân sự và định biên chức vụ
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <ChartBox
          ...
          sub="Tỷ trọng nhân viên theo phòng ban"
          icon={<PieIcon className="text-orange-500" />}
        >
          <div className="h-87.5 mt-6">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <RechartsTooltip
                  content={<CustomTooltip content="Nhân sự" />}
                />
                <Legend
        ...
        <ChartBox
          title="Định biên vị trí"
          sub="Số lượng chức vụ theo từng đơn vị"
          ...
          <div className="h-87.5 mt-6">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={statistics.positionsArray}>
                ...
                <RechartsTooltip
                  cursor={{ fill: "#f8fafc" }}
                  content={<CustomTooltip content="Chức vụ" />}
                />
                <Bar dataKey="value" radius={[10, 10, 0, 0]} barSize={40}>
                  {statistics.positionsArray.map(
                    (entry: any, index: number) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={COLORS[index % COLORS.length]}
      ...
      <ChartBox
        ...
        sub="Phân tích tỷ lệ trung bình toàn hệ thống"
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mt-8">
          <MetricCard
            ...
            value={(
              statistics.totalEmployees / statistics.totalDepartments
            ).toFixed(1)}
          ...
          <MetricCard
            label="Chức vụ/Phòng"
            value={(
              statistics.totalPositions / statistics.totalDepartments
            ).toFixed(1)}
            color="bg-purple-50 text-purple-600"
          />
          <MetricCard
            label="Nhân viên/Chức vụ"
            value={(
              statistics.totalEmployees / statistics.totalPositions
            ).toFixed(1)}
