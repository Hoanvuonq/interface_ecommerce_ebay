...
export const useWishlist = () => {
     ...
     * Tạo wishlist mới
     */
    const createWishlist = useCallback(async (requestData: CreateWishlistRequest) => {
     ...
     * Cập nhật wishlist
     */
    const updateWishlist = useCallback(async (wishlistId: string, requestData: UpdateWishlistRequest) => {
        return handleRequest(
     ...
     */
    const getWishlistById = useCallback(async (wishlistId: string) => {
     ...
     */
    const getBuyerWishlists = useCallback(async (params?: WishlistQueryParams) => {
     ...
     */
    const setAsDefault = useCallback(async (wishlistId: string) => {
        return handleRequest(
     ...
     */
    const addToWishlist = useCallback(async (wishlistId: string, requestData: AddToWishlistRequest) => {
        return handleRequest(
     ...
     */
    const quickAddToWishlist = useCallback(async (variantId: string, quantity: number = 1) => {
        return handleRequest(
     ...
     */
    const removeFromWishlist = useCallback(async (wishlistId: string, variantId: string) => {
        return handleRequest(
     ...
     */
    const getWishlistItems = useCallback(async (wishlistId: string) => {
    ...
    const toggleWishlist = useCallback(async (variantId: string) => {
        try {
            ...
            if (inWishlist) {
                ...
                await wishlistService.removeFromWishlist(defaultWishlist.id, variantId);
                console.log('✅ Removed successfully');
                return { success: true, added: false };
            } else {
                // Add to wishlist
                console.log('➕ Adding to wishlist...');
                await wishlistService.addToDefaultWishlist({ variantId, quantity: 1 });
                console.log('✅ Added successfully');
                return { success: true, added: true };
            }
        } catch (err: any) {
            console.error('❌ Error toggling wishlist:', err);
            const errorMessage = typeof err === 'string' ? err : err?.message || 'Có lỗi xảy ra';
            setError(errorMessage);
            return { success: false, error: errorMessage };
        }
    }, [isProductInWishlist]);
    ...
    return {
        ...
        getPriceTargetMetItems: useCallback(async () => {
            return handleRequest(
                () => wishlistService.getPriceTargetMetItems()
            );
        }, [handleRequest]),

        // Wishlist Sharing
        regenerateShareToken: useCallback(async (wishlistId: string) => {
            return handleRequest(
