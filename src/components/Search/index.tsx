...
export const Search: React.FC<ISearch> = ({
  ...
  if (!mounted)
    return (
      <div
        className={cn("h-12 bg-gray-100 rounded-xl animate-pulse", className)}
      />
    );
      ...
      <div className="relative flex-1 flex items-center">
        ...
        {isDropdownOpen &&
          (history.length > 0 ||
            ...
            <div
              className={cn(
                "absolute top-[calc(100%+12px)] left-0 w-full bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)]",
                "rounded-3xl border border-gray-100 overflow-hidden z-9999 animate-in fade-in slide-in-from-top-3 duration-300",
              )}
            >
                    ...
                    {history.map((item, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3 text-sm font-bold  text-gray-600">
                          <History
                            size={16}
                            className=" text-gray-300 group-hover/item:text-orange-500 transition-colors"
                          />
                          {item}
                        </div>
                        <X
                          ...
                          className=" text-gray-300 hover:text-rose-500 opacity-0 group-hover/item:opacity-100 transition-all"
                        />
                      </div>
                    ))}
                  </div>
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <div className="flex items-center gap-3 text-sm  text-gray-700 font-bold">
                            <SearchIcon
                              size={16}
                              className=" text-gray-300 group-hover/opt:text-orange-500"
                            />
                            {labelText}
                          </div>
                          {opt.item?.searchCount > 0 && (
                            <span className="ml-7 text-[10px]  text-gray-400 font-medium italic">
                              Hơn {opt.item.searchCount.toLocaleString()} lượt
                              tìm kiếm
                            </span>
                          )}
                        </div>
                        <Zap
                          size={14}
                          className="text-yellow-400 opacity-0 group-hover/opt:opacity-100 transition-all"
                        />
                      </div>
                    );
                  })}
