...
export function findOldAddressByNewWardCode(
  ...
  if (newWardName && newProvinceCode) {
    ...
    addressData.forEach((item) => {
      if (item.type === "table" && item.name === "provinces" && item.data) {
        ...
        const province = provinces.find(
          (p) => p.province_code === newProvinceCode,
        );
        if (province) {
    ...
    const mappingByName = mappings.find(
      (m) =>
        m.new_ward_name === newWardName && m.new_province_name === provinceName,
    );
...
export function getWardFromProvinceAndDistrict(
  ...
  const mapping = mappings.find(
    (m) =>
      ...
      (m.old_district_name === districtName ||
        m.new_ward_name?.includes(districtName)),
  );
  ...
  const firstWard = wards.find(
    (w) => w.province_code === province.province_code,
  );
